...
`check`는 CRAP과 mutation을 별도 프로세스에서 병렬로 실행합니다. `--execution-mode sequential`을 붙이면 순차 실행합니다. 두 작업은 독립된 임시 폴더를 사용하고, 실행 오류나 취소가 발생하면 다른 작업도 중단·정리합니다. 점수 미달은 실행 오류와 구분하며 두 결과를 함께 보고합니다.

검사할 프로젝트의 외부 패키지는 통합 설치의 `--python-requirements`로 `.sentinel-deps`에 준비합니다. 이 폴더는 각 작업 사본에 복사되어 테스트의 패키지 검색 경로에 연결됩니다. 사용자의 기존 `.venv`는 복사하지 않습니다. 병렬 실행에서는 두 의존성 사본의 저장 공간도 동시에 필요합니다.
